...
ShopSense is a full-stack fashion e-commerce platform built with Next.js 13 (App Router), MongoDB, Supabase, and Stripe. It provides a customer storefront and an admin panel for product and order management. The project also includes an AI layer for semantic product search/recommendation using Sentence Transformers and FAISS, plus a DistilBERT-based review sentiment service.

The Next.js application contains the storefront and backend API routes in the same codebase. The AI functionality runs as a separate local Python/FastAPI service, so the project can be developed and run without Docker.

This document explains the complete application architecture, AI recommendation/search flow, environment configuration, local setup, API structure, and important implementation details.
...
ShopSense adds two AI capabilities:

- **Semantic product search and recommendation:** Product name, description, category, and other relevant attributes are converted into semantic embeddings using the **Sentence Transformers `all-MiniLM-L6-v2`** model. The embeddings are indexed with **FAISS** and ranked using similarity search to return relevant Top-K products.
- **Review sentiment analysis:** Customer review text is processed by a **DistilBERT** model to classify sentiment and provide a confidence score.

The semantic search is designed to match meaning rather than relying only on exact keyword overlap. For example, a query such as `comfortable clothes for summer travel` can retrieve products whose descriptions express similar concepts even when the exact words are different.
...
all-MiniLM-L6-v2           Review Sentiment
       |
       v
Semantic Embedding
       |
...
Top-K Relevant Products
       |
       v
ShopSense Search Results
```

The AI service is independent from the Next.js server. Therefore, the application can be run locally with two processes: the Next.js development server on port `3000` and the Python AI service on port `8000`.
...
**## 5.1 AI Service Environment Variable

Add the following variable to the Next.js `.env.local`:

| Variable | Used by | Purpose |
|---|---|---|
| `AI_SERVICE_URL` | Next.js server | Base URL of the local AI/FastAPI service, normally `http://localhost:8000` |

The Python AI service itself does not require an OpenAI API key. The semantic search uses the local Sentence Transformer model and FAISS, while sentiment analysis uses DistilBERT.
...
Response: \`{ "success": true, "id": "cs_test\_..." }\`. The frontend uses this id with Stripe's client-side redirect helper; Stripe then sends the customer back to SERVER/checkout?status=success or ?status=cancel.
...
**## 7. Running ShopSense Locally (Without Docker)

ShopSense can be run completely without Docker. You need Node.js/npm for the Next.js application and Python 3 for the AI service.
...
- Stripe account in test mode for payments

### 7.2 Configure the Next.js application
...
The first startup may download the required Hugging Face model files.
...
Open a third terminal, or return to the project root:
...
### 7.5 Verify the AI search

Open ShopSense in the browser and use the search box in the navigation bar.
...
The query is sent to the AI service, converted into a semantic embedding by `all-MiniLM-L6-v2`, searched against the FAISS product index, and returned as ranked product results.

### 7.6 Database and external services

Before testing the complete checkout flow:

1. Set a valid MongoDB connection string in `DATABASE_URL`.
2. Create the Supabase storage bucket specified by `NEXT_PUBLIC_SUPABASE_STORAGE_BUCKET`.
3. Configure the required Supabase storage policies for product images.
4. Use Stripe test-mode keys in `PUBLIC_KEY` and `PRIVATE_KEY`.
5. Start both the AI service and Next.js server.
...
For the AI service, run Uvicorn without `--reload`:
